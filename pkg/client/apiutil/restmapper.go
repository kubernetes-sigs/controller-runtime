...
type mapper struct {
	...
	supportsAggregatedDiscovery          atomic.Bool
	checkedIfSupportsAggregatedDiscovery atomic.Bool
...
func (m *mapper) addKnownGroupAndReload(groupName string, versions ...string) error {
	...
	//
	// Calling this also tells us if the server supports aggregated discovery, which allows
	// loading everything with two api calls which we assume is overall cheaper, hence we always
	// do this once optimistically.
	if len(versions) == 0 || !m.checkedIfSupportsAggregatedDiscovery.Load() {
		apiGroup, didLookup, err := m.findAPIGroupByNameAndMaybeAggregatedDiscovery(groupName)
		if err != nil {
			return err
		}
		...
		if m.supportsAggregatedDiscovery.Load() && didLookup {
	...
	m.mu.Lock()
	defer m.mu.Unlock()
	m.addGroupVersionResourcesToCache(groupVersionResources)
	return nil
}

// addGroupVersionResourcesToCache does what the name suggests. The mutex must be held when
// calling it.
func (m *mapper) addGroupVersionResourcesToCache(gvr map[schema.GroupVersion]*metav1.APIResourceList) {
...
// findAPIGroupByNameAndMaybeAggregatedDiscovery tries to finds the passed apiGroup.
// If the server supports aggregated discovery, it will always perform that.
func (m *mapper) findAPIGroupByNameAndMaybeAggregatedDiscovery(groupName string) (*metav1.APIGroup, bool, error) {
	...
	apiGroups, maybeResources, failedGVs, err := m.client.GroupsAndMaybeResources()
	if err != nil {
	...
	m.checkedIfSupportsAggregatedDiscovery.Store(true)
	if len(maybeResources) > 0 {
		m.supportsAggregatedDiscovery.Store(true)
	}
	for failedGV := range failedGVs {
		delete(maybeResources, failedGV)
	}
	m.addGroupVersionResourcesToCache(maybeResources)
	m.mu.Unlock()
	...
	return m.apiGroups[groupName], true, nil
}
