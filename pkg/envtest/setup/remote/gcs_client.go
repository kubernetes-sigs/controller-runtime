...
const (
	// DefaultBucket is the default GCS bucket to fetch from, if using the old and deprecated GCS source
	//
	// Deprecated: Please use the HTTP client and its default options instead
	DefaultBucket = "kubebuilder-tools"

	// DefaultServer is the default GCS-like storage server to fetch from, if using the old and deprecated GCS source
	//
	// Deprecated: Please use the HTTP client and its default options instead
	DefaultServer = "storage.googleapis.com"
)
...
func (c *GCSClient) FetchSum(ctx context.Context, ver versions.Concrete, pl versions.Platform) (*versions.Hash, error) {
	...
	req, err := http.NewRequestWithContext(ctx, "GET", loc.String(), nil)
	if err != nil {
		return nil, fmt.Errorf("unable to construct request to fetch metadata for %s: %w", itemName, err)
	}
	resp, err := http.DefaultClient.Do(req)
	if err != nil {
		return nil, fmt.Errorf("unable to fetch metadata for %s: %w", itemName, err)
	}
	defer resp.Body.Close()

	if resp.StatusCode != 200 {
		return nil, fmt.Errorf("unable fetch metadata for %s -- got status %q from GCS", itemName, resp.Status)
	}

	var item bucketObject
	if err := json.NewDecoder(resp.Body).Decode(&item); err != nil {
		return nil, fmt.Errorf("unable to unmarshal metadata for %s: %w", itemName, err)
	}
...
// LatestVersion returns the latest version of the tools that matches the given spec and platform.
func (c *GCSClient) LatestVersion(ctx context.Context, spec versions.Spec, platform versions.Platform) (versions.Concrete, error) {
