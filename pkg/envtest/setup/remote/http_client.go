...
// FetchSum fetches the checksum for the given concrete version & platform
func (c *HTTPClient) FetchSum(ctx context.Context, version versions.Concrete, platform versions.Platform) (*versions.Hash, error) {
	index, err := c.getIndex(ctx)
	if err != nil {
		return nil, err
	}
	...
	return nil, fmt.Errorf("unable to find archive for %s (%s,%s)", version, platform.OS, platform.Arch)
}

// LatestVersion returns the latest version that matches the given spec and platform.
func (c *HTTPClient) LatestVersion(ctx context.Context, spec versions.Spec, platform versions.Platform) (versions.Concrete, error) {
