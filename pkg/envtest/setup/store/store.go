...
func (s *Store) List(ctx context.Context, matching Filter) ([]Item, error) {
	...
	if err := s.eachItem(ctx, matching, func(_ string, item Item) {
		...
		return nil, fmt.Errorf("%w in store: %w", ErrUnableToList, err)
	}
...
// ErrUnableToList signals that something went wrong when listing items in the store.
// It is typically wrapped together with some more details.
var ErrUnableToList = errors.New("unable to list version-platform pairs")
...
func (s *Store) Remove(ctx context.Context, matching Filter) ([]Item, error) {
	...
	if err := s.eachItem(ctx, matching, func(name string, item Item) {
		...
		if err := s.removeItem(s.unpackedPath(name)); err != nil {
			...
			savedErr = errors.Join(savedErr, fmt.Errorf("unable to remove version-platform pair %s (dir %s): %w", item, name, err))
			return // don't mark this as removed in the report
		}
		removed = append(removed, item)
	}); err != nil {
		return removed, fmt.Errorf("%w to figure out what to delete: %w", ErrUnableToList, err)
	}
	if savedErr != nil {
