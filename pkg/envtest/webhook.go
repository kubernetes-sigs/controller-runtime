...
func (o *WebhookInstallOptions) setupCA() error {
	...
	if err := ioutil.WriteFile(filepath.Join(localServingCertsDir, "tls.crt"), certData, 0600); err != nil {
		return fmt.Errorf("unable to write webhook serving cert to disk: %v", err)
	}
	if err := ioutil.WriteFile(filepath.Join(localServingCertsDir, "tls.key"), keyData, 0600); err != nil {
