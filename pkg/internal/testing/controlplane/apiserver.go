...
func (s *APIServer) populateAPIServerCerts() error {
	...
	if err := ioutil.WriteFile(filepath.Join(s.CertDir, "apiserver.crt"), certData, 0600); err != nil {
		return err
	}
	if err := ioutil.WriteFile(filepath.Join(s.CertDir, "apiserver.key"), keyData, 0600); err != nil {
		return err
	}
	...
	if err := ioutil.WriteFile(filepath.Join(s.CertDir, saCertFile), saCert, 0600); err != nil {
		return err
	}
	return ioutil.WriteFile(filepath.Join(s.CertDir, saKeyFile), saKey, 0600)
}
